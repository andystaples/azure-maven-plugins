...
public class ConfigMojo extends AbstractWebAppMojo {
    ...
    private WebAppConfiguration getRuntimeConfiguration(WebAppConfiguration configuration, boolean initial)
        ...
        builder.os(osEnu);
        if (initial || pricingTierNotSupport(osEnu, configuration.getPricingTier())) {
            String defaultPricingTier = configuration.getPricingTierOrDefault();
            final List<String> availablePriceList = getAvailablePricingTierList(osEnu);
        ...
        switch (os.toLowerCase()) {
            case "linux":
            case "windows":
                builder = getRuntimeConfigurationForWindowsOrLinux(osEnu, builder, configuration);
                break;
            case "docker":
    ...
    private static boolean pricingTierNotSupport(OperatingSystem parseOperationSystem, PricingTier pricingTier) {
    ...
    private WebAppConfiguration.WebAppConfigurationBuilder<?, ?> getRuntimeConfigurationForWindowsOrLinux(OperatingSystem os,
        ...
        if (isJarProject()) {
            return builder.javaVersion(javaVersion).webContainer(WebContainer.JAVA_SE);
        }
    ...
    private static List<String> getAvailablePricingTierList(OperatingSystem operatingSystem) {
    ...
    private static WebAppConfiguration getConfigurationFromExisting(IWebApp webapp, IAppServicePlan servicePlan,
        ...
        if (servicePlan != null) {
            builder.pricingTier(servicePlan.entity().getPricingTier());
